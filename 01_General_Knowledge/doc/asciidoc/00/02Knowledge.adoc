...
This is the flag for compilation.

[title=": References", options="header", caption="Table {chapter_num}-{counter:table_num}", id=table_2-1, cols="2,3,7", align=center]
|=================
...
|-_UNUSED-but-set-variable|Warns when a variable is written to but its value is never read
...
* [.font-courier]#<build flags>#: Compiler options listed in <<Chapter_2-1-1, Compilation flags>>. These options are optional.
...
[[Chapter_2-4-1-3_Step_2]]
*Step 2*: Create a static library:

You can have multiple object files ([.font-courier]#.o#) built in <<Chapter_2-4-1-3_Step_1, Step 1>>. Please run this command below in a terminal to create the static library from your object files.
...
Whereas, [.font-courier]#<path to output executable># and [.font-courier]#<executable file># are defined in <<Chapter_2-4-1-4, How to build an application linked with static libraries>>.
...
With those pros and cons mentioned in <<Chapter_2-4-1-6, Pros and cons>>, these usages can be considered:
...
* For other options, please refer to <<Chapter_2-4-1-3_Step_1, Create object file for static library>>.

After running that command, you will have the object file [.font-courier]#<lib object>.o# in [.font-courier]#<path to lib object>#.

[[Chapter_2-4-2-3_Step_2]]
*Step 2*: Create a shared library:

You can have multiple object files ([.font-courier]#.o#) built in <<Chapter_2-4-2-3_Step_1, Step 1>>. Please run this command below in a terminal to create the shared library from your object files.
...
Once you have your shared library in [.font-courier]#<path to shared library>#, and your source files [.font-courier]#<source file x># ([.font-courier]#x#: 1, 2, ..., n) in their respective directories [.font-courier]#<path to source file x>#, let's just create the executable file linked with the shared library.
...
For all the options, please refer to <<Chapter_2-4-1-4, How to build an application linked with static libraries>>.
...
[title=": Searched directories of descending priorities", caption="Table {chapter_num}-{counter:table_num}", id=table_2-2, cols="1,9", align=center]
|=================
|No.|Directory
...
For (2) in the <<table_2-2, Table 2-2>>, you can add this option [.font-courier]#-Wl,-rpath,<path to shared library># to the build command line in <<Chapter_2-4-2-4, How to build an application linked with shared libraries>>.
...
Whereas, [.font-courier]#<path to output executable># and [.font-courier]#<executable file># are defined in <<Chapter_2-4-2-4, How to build an application linked with shared libraries>>.
...
With those pros and cons mentioned in <<Chapter_2-4-2-6, Pros and cons>>, these usages can be considered:
